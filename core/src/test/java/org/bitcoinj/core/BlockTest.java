...
public class BlockTest {
    ...
    public final Transaction getSaveTheChainTransaction() throws Exception {
        URL bigBlockResource = getClass().getResource("savethechain.tx");
        String txHex = Resources.toString(bigBlockResource, StandardCharsets.UTF_8);
        byte[] txBytes = HEX.decode(txHex);
        return PARAMS.getDefaultSerializer().makeTransaction(txBytes);
    }

    public Block bigBlock;

    public final void setBigBlock() throws Exception {
        Block genesis = PARAMS.getGenesisBlock();
        int height = Block.BLOCK_HEIGHT_GENESIS+1;
        long version = Block.BLOCK_VERSION_BIP65;
        Sha256Hash prevHash = genesis.getHash();
        Transaction tx = getSaveTheChainTransaction();
        List<Transaction> txs = new ArrayList<Transaction>();
        txs.add(tx);
        Sha256Hash merkleRoot = tx.getHash();
        long time = 1494096804; 
        long target = genesis.getDifficultyTarget();
        long nonce = 0;
        if (bigBlock == null) {
            bigBlock = new Block(PARAMS, version, prevHash, merkleRoot, time, target, nonce, txs);
            bigBlock.solve();
        }
    }
    ...
    /*
    @Test
    public void testBigBlockVerification() throws Exception {
        Block bigBlock = getBigBlock();
        int height = Block.BLOCK_HEIGHT_GENESIS+1;
        bigBlock.verify(height, EnumSet.noneOf(Block.VerifyFlag.class));
        assertTrue(bigBlock.bitcoinSerialize().length > 1000000);
    }
    */
    ...
    public void testUpdateLength() {
        ...
        int origTransactionLength = 8 + 2 + 8 + 1 + 10 + 40 + 1 + 1;
        assertEquals(tx.unsafeBitcoinSerialize().length, tx.length);
        assertEquals(origTransactionLength, tx.length);
        ...
        assertEquals(tx.length, origTransactionLength + 1);
        ...
        block.getTransactions().get(1).addInput(new TransactionInput(params, null, new byte[] {(byte) ScriptOpCodes.OP_FALSE},
                new TransactionOutPoint(params, 0, Sha256Hash.of(new byte[] { 1 }))));
        assertEquals(block.length, origBlockLen + tx.length);
        assertEquals(tx.length, origTransactionLength + 41); // - 1 + 40 + 1 + 1
    }
